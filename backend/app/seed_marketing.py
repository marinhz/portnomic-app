...
async def seed_marketing(db: AsyncSession) -> None:
    ...
    # Create Leakage Detector anomalies for selected DAs (AI expense audit demo data)
    ...
    if das_to_seed and default_email:
        for i, da in enumerate(das_to_seed[:4]):  # Seed anomalies for first 4 DAs
            # Pick 2 different anomalies per DA for variety
            idx1, idx2 = (i * 2) % len(ANOMALY_SAMPLES), (i * 2 + 1) % len(ANOMALY_SAMPLES)
            samples_for_da = [ANOMALY_SAMPLES[idx1], ANOMALY_SAMPLES[idx2]]
            for sample in samples_for_da:
                a = Anomaly(
                    tenant_id=tenant_id,
                ...
                db.add(a)
        await db.flush()
        print("  Created Leakage Detector anomalies for 4 DAs")
    else:
        print("  Skipped anomaly seed (no DAs or emails)")
